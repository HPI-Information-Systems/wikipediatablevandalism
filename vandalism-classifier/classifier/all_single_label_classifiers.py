...
class SingleLabelClassifierRunner:
    ...
    def run_all(self):
        ...
        for tag in tags:
            ...
            preprocessor = Preprocessor(self.features, [tag], vandalism_sample_rate=0.3)
            ...
            plt.savefig(os.path.join(self.report_output, tag_name + '_train_pr_auc.png'))
            plt.show()
            print("PR-AUC", pr_auc)

            plt, roc_auc = create_receiver_operation_characteristic_graph(y_train, y_train_predict_proba[:, 1])
            plt.savefig(os.path.join(self.report_output, tag_name + '_train_roc_auc.png'))
            plt.show()
            ...
            plt.savefig(os.path.join(self.report_output, tag_name + '_test_pr_auc.png'))
            plt.show()
            print("PR-AUC", pr_auc)

            plt, roc_auc = create_receiver_operation_characteristic_graph(y, y_predict_proba[:, 1])
            plt.savefig(os.path.join(self.report_output, tag_name + '_test_roc_auc.png'))
            plt.show()
            print("ROC-AUC", roc_auc)
            ...
            plt.savefig(os.path.join(self.report_output, tag_name + '_false_negatives.png'))
            plt.show()

            # Feature importance
            plt = create_feature_importance_graph(clf, data.labels)
            plt.savefig(os.path.join(self.report_output, tag_name + '_test_feature_importance.png'))
            plt.show()
