...
from preprocessing.preprocessor import DataSet
...
class SingleLabelClassifier:
    ...
    def __init__(self, dataset: DataSet, trees: int, k_folds=10):
    ...
    def cross_validate(self):
        ...
        self.clf = RandomForestClassifier(n_estimators=self.trees, n_jobs=-1)
        scores = cross_validate(self.clf, self.dataset.X_train, self.dataset.y_train,
    ...
    def train_predict(self):
        ...
        self.clf = RandomForestClassifier(n_estimators=self.trees, n_jobs=-1)
        classes = cross_val_predict(self.clf, self.dataset.X_train, self.dataset.y_train,
                                    cv=self.k_folds, n_jobs=-1, method='predict')
        probabilities = cross_val_predict(self.clf, self.dataset.X_train, self.dataset.y_train,
    ...
    def test_predict(self):
        ...
        classes = self.clf.predict(self.dataset.X_test)
        probabilities = self.clf.predict_proba(self.dataset.X_test)
        return classes, probabilities
