...
public class DiffMatchPatchUtil {
   ...
   * to match the expected contents. (0.0 = perfection, 1.0 = very loose).  Note that
   ...
   * @return Linked List of Diff objects.
   */
  public LinkedList<Diff> diffCompute(String text1, String text2,
      boolean checklines, long deadline) {
   ...
   * @return Linked List of Diff objects.
   */
  public LinkedList<Diff> diffLineMode(String text1, String text2,
      long deadline) {
   ...
   * @param lineArray List of unique strings.
   */
  private void diffCharsToLines(List<Diff> diffs,
      List<String> lineArray) {
    StringBuilder text;
    for (Diff diff : diffs) {
   ...
   * second string.
   */
  private int diffCommonOverlap(String text1, String text2) {
    // Cache the text lengths to prevent multiple calls.
    int text1_length = text1.length();
    int text2_length = text2.length();
   ...
   * the prefix of shorttext, the suffix of shorttext and the common middle.  Or null if there was
   * no match.
   */
   ...
   * fine. Note that this function is case-sensitive.  Thus "%3f" would not be unescaped.  But this
   * is ok because it is only called with the output of URLEncoder.encode which returns uppercase
   * hex.
   *
   * Example: "%3F" -> "?", "%24" -> "$", etc.
   *
